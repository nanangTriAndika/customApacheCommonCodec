 ...
 * @since Mar 25, 2010
 */
public class ApacheBase64Codec13Test {
    ...
    /**
     * Tests to make sure ApacheBase64's implementation of the org.apache.commons.codec.Encoder
    ...
    public void testEncoder() throws EncoderException {
        final Encoder enc = new ApacheBase64();
    ...
    /**
     * Tests to make sure ApacheBase64's implementation of the org.apache.commons.codec.Decoder
     * interface is behaving identical to commons-codec-1.3.jar.
     *
    ...
    public void testDecoder() throws DecoderException {
        final Decoder dec = new ApacheBase64();
        for (int i = 0; i < STRINGS.length; i++) {
            if (STRINGS[i] != null) {
                final byte[] base64 = utf8(STRINGS[i]);
                final byte[] binary = BYTES[i];
    ...
    /**
     * Tests to make sure ApacheBase64's implementation of the org.apache.commons.codec.BinaryEncoder
     * interface is behaving identical to commons-codec-1.3.jar.
     *
     * @throws EncoderException problem
     */
    ...
    public void testBinaryEncoder() throws EncoderException {
        final BinaryEncoder enc = new ApacheBase64();
        for (int i = 0; i < STRINGS.length; i++) {
            if (STRINGS[i] != null) {
                final byte[] base64 = utf8(STRINGS[i]);
                final byte[] binary = BYTES[i];
    ...
    /**
     * Tests to make sure ApacheBase64's implementation of the org.apache.commons.codec.BinaryDecoder
     * interface is behaving identical to commons-codec-1.3.jar.
     *
     * @throws DecoderException problem
     */
    ...
    public void testBinaryDecoder() throws DecoderException {
        final BinaryDecoder dec = new ApacheBase64();
        for (int i = 0; i < STRINGS.length; i++) {
            if (STRINGS[i] != null) {
                final byte[] base64 = utf8(STRINGS[i]);
                final byte[] binary = BYTES[i];
    ...
    public void testStaticEncode() throws EncoderException {
        for (int i = 0; i < STRINGS.length; i++) {
            if (STRINGS[i] != null) {
                final byte[] base64 = utf8(STRINGS[i]);
                final byte[] binary = BYTES[i];
                final boolean b = Arrays.equals(base64, ApacheBase64.encodeBase64(binary));
                assertTrue("static ApacheBase64.encodeBase64() test-" + i, b);
            }
        }
    }

    /**
     * Tests to make sure ApacheBase64's implementation of ApacheBase64.decodeBase64()
     * static method is behaving identical to commons-codec-1.3.jar.
     *
     * @throws DecoderException problem
     */
    ...
    public void testStaticDecode() throws DecoderException {
        for (int i = 0; i < STRINGS.length; i++) {
            if (STRINGS[i] != null) {
                final byte[] base64 = utf8(STRINGS[i]);
                final byte[] binary = BYTES[i];
                final boolean b = Arrays.equals(binary, ApacheBase64.decodeBase64(base64));
                assertTrue("static ApacheBase64.decodeBase64() test-" + i, b);
            }
        }
    }

    /**
     * Tests to make sure ApacheBase64's implementation of ApacheBase64.encodeBase64Chunked()
     * static method is behaving identical to commons-codec-1.3.jar.
     *
     * @throws EncoderException problem
     */
    ...
    public void testStaticEncodeChunked() throws EncoderException {
        for (int i = 0; i < STRINGS.length; i++) {
            if (STRINGS[i] != null) {
                ...
                final byte[] binary = BYTES[i];
                final boolean b = Arrays.equals(base64Chunked, ApacheBase64.encodeBase64Chunked(binary));
                assertTrue("static ApacheBase64.encodeBase64Chunked() test-" + i, b);
            }
        }
    }
    ...
    public void testStaticDecodeChunked() throws DecoderException {
        for (int i = 0; i < STRINGS.length; i++) {
            if (STRINGS[i] != null) {
                final byte[] base64Chunked = utf8(CHUNKED_STRINGS[i]);
                final byte[] binary = BYTES[i];
                final boolean b = Arrays.equals(binary, ApacheBase64.decodeBase64(base64Chunked));
                assertTrue("static ApacheBase64.decodeBase64Chunked() test-" + i, b);
            }
        }
    }
